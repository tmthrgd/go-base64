...
type decode struct {
	*asm.Asm
	...
	validZero, validNine, validLowerA, validLowerZ, validPlus, validSlash asm.Operand

	baseLowerA, baseUpperA, baseZero, basePlus, baseSlash asm.Operand

	toLower, sign, upperZ, mergeBytes, mergeWords asm.Operand
}

func (d *decode) Convert() {
	/*d.Vpxor(asm.X2, asm.X1, d.sign)
	d.Vpor(asm.X3, asm.X2, d.toLower)

	d.Vpcmpgtb(asm.X4, d.validZero, asm.X2)
	d.Vpcmpgtb(asm.X5, asm.X2, d.validNine)
	d.Vpcmpgtb(asm.X6, d.validLowerA, asm.X3)
	d.Pcmpgtb(asm.X3, d.validLowerZ)
	d.Vpcmpeqb(asm.X7, asm.X1, d.validPlus)
	d.Vpcmpeqb(asm.X8, asm.X1, d.validSlash)

	d.Pand(asm.X6, asm.X5)
	d.Por(asm.X6, asm.X4)
	d.Por(asm.X6, asm.X3)
	d.Por(asm.X7, asm.X8)
	d.Pandn(asm.X7, asm.X6)

	d.Pmovmskb(asm.AX, asm.X7)

	d.Testw(asm.DX, asm.AX)
	d.Jnz(d.invalid)

	d.Pcmpgtb(asm.X2, d.upperZ)

	d.Vpblendvb(asm.X3, d.baseUpperA, d.baseLowerA, asm.X2)
	d.Vpblendvb(asm.X3, d.baseZero, asm.X3, asm.X5)

	d.Psubb(asm.X1, asm.X3)

	d.Vpblendvb(asm.X1, asm.X1, d.basePlus, asm.X7)
	d.Vpblendvb(asm.X1, asm.X1, d.baseSlash, asm.X8)*/
...
func decodeASM(a *asm.Asm) {
	...
	var b52 byte = 52 // "-X overflows byte"
	...
	upperZ := a.Data("decodeUpperZ", repeat('Z' ^ 0x80, 16))*/
	...
	d := &decode{
		...
		/*asm.X11, asm.Address(asm.R13, 48), asm.X12, valid.Offset(16), asm.Address(asm.R13, 64), asm.Address(asm.R13, 80),

		asm.Address(asm.R12, 16), asm.X13, asm.X14, asm.Address(asm.R12, 48), asm.Address(asm.R12, 60),

		asm.X15, sign, upperZ,*/asm.Address(asm.R11, 0), asm.Address(asm.R11, 16),
	}
	...
	a.Movq(asm.R11, merge.Address())
	/*a.Movq(asm.R12, base.Address())
	a.Movq(asm.R13, valid.Address())
	a.Movou(d.validZero, valid.Offset(32))
	a.Movou(d.validLowerA, valid.Offset(0))
	a.Movou(d.baseUpperA, base.Offset(0))
	a.Movou(d.baseZero, base.Offset(32))
	a.Movou(d.toLower, toLower)*/
